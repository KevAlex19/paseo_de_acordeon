...
class FavView extends StatefulWidget {
  ...
  FavView(this.base, this.title, this.filter, this.user);

  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends StateMVC<FavView> {
  MainController _con;
  List<TasktoDo> items = [];
  Map<String, List<dynamic>> favsMap = new Map<String, List<dynamic>>();
  List<String> favs;
  ...
  void initState() {
    ...
    var po = FirebaseDatabase.instance.reference();
    po.onValue.forEach((element) {
      Map<dynamic, dynamic> lista = element.snapshot.value;
      print(lista.keys.toString());
      for (var item in lista.keys) {
        ...
        if (content.length > 0) {
          for (var i = 0; i < content.length; i++) {
            DatabaseReference postRef =
                FirebaseDatabase.instance.reference().child(content[i]);
            postRef.once().then((DataSnapshot snap) {
              ...
              for (var individualKey in keys) {
                ...
                for (var comparate in favs) {
                  if (event.title==comparate) {
                    items.add(event);
                  }
                }
              }
